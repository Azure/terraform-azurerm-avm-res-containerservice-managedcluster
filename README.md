...
Description: AzAPI resource type overrides. Each value defaults to the API version tested by the scope that declares the resource.
...
Description: Retry configuration applied to every supported AzAPI resource declared by the module and its applicable submodules.
